var NewSpawnFunction = {  
    run: function(target) {  
        ...
        // 然后检查房间能量和任务列表  
        if (spawnRoom.energyAvailable < 1150 && !roomMemory.tasks.some(task => task.type === 'fillExtension')) {  
            // 推送 'fillExtension' 任务  
        ...
        var roleConfigs = [  
            ...
            { role: 'Newtransferer',    sourceRoom: sourceRooms[0], targetRoom: targetRooms[0], workLoc: [0],     maxNumber: 0, priority: 8}, 
...
function createCreepBody(role) {    //返回body
    ...
    switch (role) {  
        ...
        case 'manager':  
            ...
            break;  
        case 'upgrader':  
            body = [WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,WORK,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,CARRY,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE,MOVE];  
            break;  
        case 'builder':  
